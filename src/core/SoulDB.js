...
export class LinePicker {
    ...
    next(type) {
        if (!AI_SOUL_DB[type] || AI_SOUL_DB[type].length === 0) return "";
        if (!this.bags[type] || this.bags[type].length === 0) this.refill(type);

        for (let tries = 0; tries < 6; tries++) {
            if (this.bags[type].length === 0) this.refill(type);
            const candidate = this.bags[type].pop();
            if (candidate && candidate !== this.lastLine) {
                this.lastLine = candidate;
                return candidate;
            }
        }

        const fallback = AI_SOUL_DB[type][(Math.random() * AI_SOUL_DB[type].length) | 0] || "";
        this.lastLine = fallback;
        return fallback;
    }
}
